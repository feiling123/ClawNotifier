...
export const createAppContext = (env: CloudflareBindings): AppContext => {
  ...
  const adminPath = requireBinding(env.ADMIN_PATH, "ADMIN_PATH");
  if (!/^[A-Za-z0-9_-]{8,64}$/.test(adminPath)) {
    throw new AppError(500, "invalid_binding", "ADMIN_PATH 必须由 8-64 位字母、数字、下划线或中划线组成。");
  }

  const adminUsername = env.ADMIN_USERNAME?.trim() || "whoami";
  ...
  return {
    ...
    services: {
      ...
      delivery: new DefaultDeliveryService(deliveryLogRepository, botRepository, ilinkClient),
      health: new DefaultHealthService(env.DB, botRepository)
    }
  };
};
