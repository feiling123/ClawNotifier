...
describe("app routes", () => {
  it("should return health status", async () => {
    ...
    expect(body.data.service).toBe("ClawNotifier");
  });
  ...
  it("should accept webhook requests with shared token", async () => {
    ...
    expect(context.services.delivery.sendDelivery).toHaveBeenCalledWith("github", {
      text: "build completed",
      dedupeKey: "build-1",
      traceId: undefined,
      meta: undefined
  ...
  it("should render qrcode page via query token", async () => {
    ...
    expect(body).toContain("返回总览");
    expect(body).toContain("const fetchStatus");
  ...
  it("should render dashboard page via query token", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.headers.get("content-type")).toContain("text/html");
    expect(body).toContain("总览");
    expect(body).toContain("最近投递");
  ...
  it("should render delivery log page via query token", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.headers.get("content-type")).toContain("text/html");
    expect(body).toContain("投递记录");
    expect(body).toContain("failed");
    expect(body).toContain("github");
    expect(body).toContain("10");
    expect(body).toContain("const initialPage = \"2\"");
    expect(body).toContain("复制 API 地址");
    ...
    const scripts = Array.from(body.matchAll(/<script>([\s\S]*?)<\/script>/g)).map((match) => match[1]);
